...
library(leaps);library(Ecdat);library(car);library(lmtest);library(psych);library(ggplot2);library("ElemStatLearn")
...
# KNN  Predictions
predictions.knn.test  <- sapply (1:15,  function(k) {knn(train = knn.train ,test = knn.test ,cl = knn.train.Y,k = k)})
predictions.knn.train  <- sapply (1:15 ,  function(k) {knn(train = knn.train ,test = knn.train ,cl = knn.train.Y,k = k)})
# Compute  error  rates
errors.xs <- 1:15
...
errors  <- data.frame("K"=errors.xs,"KNN.Train"=errors.knn.train ,"KNN.Test"=errors.knn.test ,"LR.Train"=errors.lm.train ,
...
ggsave("exercise_2_8.pdf")
ggsave( "exercise_2_8.png")